...
|---|-----------|------------|
| 1 | ♪ **Musik-Player** | ♪-Symbol in der Leiste (Titel im Tooltip), Mittelklick = Play/Pause. Panel: Zurück/Play-Pause/Weiter, Lautstärke-Regler, Fortschrittsbalken mit Zeitangabe (Klick spult), Shuffle an/aus, Loop aus/Titel/Liste. |
| 2 | **Playlist-Browser** | ALLE Playlisten der Website auf einen Blick: Die BausLs, Moderne Kinderlieder **Vol. 1 + Vol. 2**, Soundtracks, Karaoke, Hörbücher, Coversongs **und das Mitsingen-Lied „Gute Laune“**. Playlist wählen → Titel werden geladen, **alle Titel laufen automatisch nacheinander** (M3U in mpv) und die Liste zeigt jeden Song zum direkten Auswählen – ein Klick springt dorthin und spielt ab dort weiter. Das Mitsingen-Lied liegt als **direktes MP3** auf der Website und startet deshalb **ohne yt-dlp**; `↗ text` öffnet die Mitsing-Seite, auf der der Text zur Musik mitläuft. „▸ alle soundcloud-playlisten laden“ holt zusätzlich immer den aktuellen Stand des Profils. Eigene URLs via yt-dlp-Import. |
...
2. Entpacken und den Ordner an den richtigen Ort legen:
   ...
   unzip familienfabrik-omarchy-plugin-1.0.0.zip \
...
| Playlist in der Übersicht anklicken | Titel laden → **ganze Playlist läuft nacheinander ab** → Titelliste erscheint |
...
(familienfabrik.at/musik): Die BausLs („Nordwind im Herzen“), Moderne
Kinderlieder **Vol. 1 und Vol. 2**, Soundtracks („1, 2, 3, Peng“ – die
Fahrrad-Gang), Karaoke (Instrumentals), Hörbücher („Lukas die kleine
Springspinne“), Coversongs (BausLs & Friends) und das Mitsingen-Lied
„Gute Laune“ – das als direktes MP3 sogar ohne yt-dlp startet. Eine Playlist
anklicken genügt: `yt-dlp` liest die Titel, das PlugIn schreibt eine
M3U-Datei und `mpv` spielt **alle Titel nacheinander** ab – der Klick auf
einen Song startet die Kette ab genau diesem Titel. Die Leiste zeigt während
der Wiedergabe Titel + Interpret via MPRIS.
...
├── manifest.json      # PlugIn-Vertrag (Schema v1, bar-widget)
...
- **Playlist lädt nicht?** Meist fehlt `yt-dlp` oder das Netz war weg. Den
  „alle soundcloud-playlisten laden“-Knopf erneut klicken; eigene URLs
...
- **Unterstützen:** <https://familienfabrik.at/unterstuetzen>
